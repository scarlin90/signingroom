...
const TEST_ROOM_ID = `test-room-${crypto.randomUUID()}`;
...
async function cleanupClient(client: WebSocket, stub: DurableObjectStub) {
  ...
  it('should enforce message rate limiting', async () => {
    ...
    const logUpdates = received.filter((m) => m.type === 'LOG_UPDATE');
    expect(logUpdates.length).toBeLessThan(15); // should be ~10 due to 10 msgs/sec limit
  ...
  it('should enforce the 100 signature upload limit safely via state injection', async () => {
    ...
    // 2. Connect normally and try to send the 101st signature
    const { client, received } = await createWebSocketClient();
    client.send(JSON.stringify({
      type: 'UPLOAD_PARTIAL',
  ...
  it('should enforce the room capacity limit (MAX_CONNECTIONS) via state injection', async () => {
    ...
    // Inject 40 dummy sessions directly into the DO's memory
    await runInDurableObject(roomStub, (instance: any) => {
      for (let i = 0; i < 40; i++) {
        // Use an object with a close() method instead of a string to prevent alarm() crashes!
    ...
    expect(closeEvent.code).toBe(4001); // "Room Full"
  });
  ...
  it('should enforce the room capacity limit (MAX_CONNECTIONS) via state injection', async () => {
    ...
    await new Promise<void>((resolve) => {
      failWs.addEventListener('close', () => resolve(), { once: true });
    });

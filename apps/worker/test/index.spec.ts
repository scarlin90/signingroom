...
describe('SigningRoom Durable Object', () => {
	...
	it.only('should wrap audit logs in a server-attested envelope for identity attribution', async () => {
        await initRoom();
        const { client, received } = await createWebSocketClient();

        try {
            // Send a standard action that triggers a log
            ...
            await vi.waitFor(() => {
                ...
                // Assert the envelope structure was generated by the server
	...
	it.only('should authorize a room reset if the correct admin token is provided', async () => {
        await initRoom({ adminToken: 'admin-secret' });
        
        // Attempt a reset with the correct token
        const initRes = await roomStub.fetch(
            new Request('http://internal/init', {
                method: 'POST',
    ...
    it.only('should reject a room reset if the wrong admin token is provided', async () => {
        await initRoom({ adminToken: 'admin-secret' });
        
        // Attempt a reset with the wrong token
        const initRes = await roomStub.fetch(
            new Request('http://internal/init', {
                method: 'POST',
	...
	it.only('should forward initialization errors (like 401 Unauthorized) from the Durable Object', async () => {
        await app.fetch(
        ...
        // Attempt to reset it with a bad token
        const request = new Request('http://localhost/api/room', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ roomId: TEST_ROOM_ID, adminToken: 'wrong-token' }),
	...
	it.only('should reject invalid admin token and lock out the specific IP after 5 failures', async () => {
        await initRoom();
        const { client, received } = await createWebSocketClient();
	...
	describe('Deep Edge Cases and Catch Blocks', () => {
        ...
        it.only('should track msgsInWindow, reset on expiry, and ignore rapid spammers', async () => {
            await initRoom();
            await runInDurableObject(roomStub, async (instance: any) => {
                ...
                instance.sessions.set(mockSocket, { 
                    id: '123', role: 'guest', msgsInWindow: 5, lastMsgTime: Date.now() - 200, ip: '1.1.1.1' 
                });
                
                await instance.handleMessage({ data: JSON.stringify({ type: 'LOG_ACTION' }) } as any, mockSocket);
                const session = instance.sessions.get(mockSocket);
                expect(session.msgsInWindow).toBe(1);

                await instance.handleMessage({ data: JSON.stringify({ type: 'LOG_ACTION' }) } as any, mockSocket);
                expect(session.msgsInWindow).toBe(2);

                await instance.handleMessage({ data: JSON.stringify({ type: 'LOG_ACTION' }) } as any, mockSocket);
                expect(session.msgsInWindow).toBe(3); 
            });
        });

        it.only('should shift auditLog array if it exceeds MAX_AUDIT_LOG_LENGTH', async () => {
            await initRoom();
            await runInDurableObject(roomStub, async (instance: any) => {
        ...
        it.only('should drop broken sockets gracefully during broadcast()', async () => {
            await initRoom();
            await runInDurableObject(roomStub, async (instance: any) => {
                const mockBrokenSocket = {
                    send: vi.fn().mockImplementation(() => { throw new Error('Broken pipe'); }),
        ...
        it.only('should ignore errors when attempting to close sockets during alarm()', async () => {
            await initRoom();
            await runInDurableObject(roomStub, async (instance: any) => {
                const mockBrokenSocket = {
                    send: vi.fn(),
        ...
        it.only('should gracefully handle socket closure errors during session resumption', async () => {
            await initRoom();
            await runInDurableObject(roomStub, async (instance: any) => {
                const mockOldSocket = {
                    send: vi.fn(),
                ...
                instance.sessions.set(mockOldSocket, { id: 'OLD_ID', role: 'guest' });

                await instance.handleSession(mockNewSocket as any, '1.1.1.1', 'OLD_ID');
        ...
        it.only('should initialize missing dictionary objects on label updates and display name changes', async () => {
            await initRoom();
            await runInDurableObject(roomStub, async (instance: any) => {
                const mockSocket = { send: vi.fn(), close: vi.fn() };
        ...
        it.only('should catch and handle errors inside AUTH_ROLE JSON parsing', async () => {
            await initRoom();
            await runInDurableObject(roomStub, async (instance: any) => {
                const mockSocket = { send: vi.fn(), close: vi.fn() };
        ...
        it.only('should clear the ipAuthFailures map if it exceeds MAX_IP_TRACKERS to prevent memory exhaustion', async () => {
            await initRoom();
	...
	describe('SigningRoom Data Chunking & Storage', () => {
		it.only('should save and load small state without chunking', async () => {
            ...
            await runInDurableObject(stub, async (instance: any) => {
                // Create a small payload using the new dictionary structure
                ...
                // Verify loadRoomState reassembles it correctly

...
test.describe('Web Component / Embedded Integration', () => {
  ...
  test('Widget should mount via programmatic injection and allow external guest entry', async ({
    browser,
  }) => {
  ...
  test('Should emit signingError event (PSBT_INVALID) when injected with bad data', async ({
    page,
  }) => {
  ...
  test('Should emit transactionFinalized event with full payload after threshold is reached', async ({
    page,
    context,
  }) => {
    ...
    await expect(
      frame.getByRole('button', { name: /Waiting for Signatures \(1 \/ 3\)/i }),
    ).toBeVisible();
    ...
    await expect(
      frame.getByRole('button', { name: /Waiting for Signatures \(2 \/ 3\)/i }),
    ).toBeVisible();
    ...
    await expect(async () => {
      const copyEvent = messages.find(
        (m) => m.action === 'dataCopied' && m.payload.dataType === 'final-hex',
      );
  ...
  test('Host page should successfully mount widget into Guest Mode via the Guest Flow form', async ({
    browser,
  }) => {
    // --- Setup: Multi-context browser environment ---
    const hostCtx = await browser.newContext();
  ...
  test('Should emit complete UI and Privacy telemetry events to the host', async ({
    page,
    context,
  }) => {
    ...
    await expect(async () => {
      expect(
        messages.find((m) => m.action === 'modalViewed' && m.payload.modalName.includes('Privacy')),
      ).toBeDefined();
    }).toPass({ timeout: 5000 });
  ...
  test('Should detect brute force attempts and emit securityAlert (access_denied) to the host', async ({
    browser,
  }) => {
    const hostCtx = await browser.newContext();
    const guestCtx = await browser.newContext();
    ...
    await hostFrame
      .getByRole('button', { name: 'Hidden for Privacy' })
      .first()
      .click({ force: true });
    await hostFrame.getByRole('button', { name: 'Reveal All' }).click({ force: true });
    const roomId = (
  ...
  test('Should emit participantPresence and cross-reference signatureReceived events between users', async ({
    ...
    let participantId = '';
    await expect(async () => {
      const guestJoins = hostMessages.filter(
        (m) =>
  ...
  test('Extended UI Interactions & Auditing Telemetry', async ({ page, context }) => {
    ...
    await frame.getByRole('button', { name: 'Verify', exact: true }).first().click();
    ...
    const pdfDownloadPromise = page
      .waitForEvent('download', { timeout: 10000 })
      .catch(() => console.log('No native download caught'));
    ...
    const psbtDownloadPromise = page
      .waitForEvent('download', { timeout: 10000 })
      .catch(() => console.log('No native download caught'));
    await frame
      .getByText(/Download PSBT/i)
      .first()
      .click();
